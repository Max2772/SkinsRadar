...
![Логотип SkinsRadar](../assets/icon_white.png)  
![Python](https://img.shields.io/badge/python-3.12-blue.svg)

SkinsRadar — это мощный инструмент для мониторинга скинов CS2 на [steamcommunity.com](https://steamcommunity.com) 🚀. Он позволяет отслеживать и анализировать цены на скины, обходить ограничения Steam на количество запросов с помощью прокси и находить выгодные предложения для перепродажи или других целей. **Регистрация или вход в аккаунт не требуются** 🔑 — начинайте сразу!
...
- **Настройки** ⚙️:
  ...
  - 📂 Добавление прокси через проводник (кнопка `Обновить прокси`).
  - 🗑️ Очистка базы данных прокси (кнопка `Очистить базу прокси`).
  - 🗄️ Обновление или создание базы данных предметов CS2 (кнопка `Обновить базу скинов`).
...
3. **Настройте прокси** (опционально) 🌐:
   - Используйте кнопку `Обновить прокси` для добавления прокси из текстового файла.
   - Приложение автоматически отфильтрует рабочие прокси и исключит дубликаты.
4. **Инициализируйте базу скинов** (опционально) 🗄️:
   - Используйте кнопку `Обновить базу скинов` для создания или обновления базы данных предметов CS2.
...
4. **Настройки** ⚙️:
   ...
   - Добавьте прокси через `Обновить прокси` или обновите базу данных с помощью `Обновить базу скинов`.

## 📸 Скриншоты и Логи

### Скриншоты 🖼️
Ниже приведены placeholders для основных окон SkinsRadar. Замените их реальными скриншотами, обновив пути к изображениям.
...
> **Примечание**: Чтобы добавить скриншоты, поместите файлы изображений (например, `browser_mode.png`, `radar_mode.png`, `settings.png`) в директорию `assets/screenshots/` и обновите пути выше.
...
Пожалуйста, следуйте стилю кода (используйте `black` и `flake8` для форматирования и линтинга).
